...
vuln_class: SSRF
source_url: https://eib.hashnode.dev/crafting-a-full-read-ssrf-a-journey-through-oauth-dcr-open-url-redirects-and-path-normalization
source_author: @eib_____ (X hunter)
source_date: 2026-04
...
# Derived technique from case: OAuth Dynamic Client Registration + Open Redirect to Full-Read SSRF

## 核心思路

Chain": - /url: https://eib.hashnode.dev/crafting-a-full-read-ssrf-a-journey-through-oauth-dcr-open-url-redirects-and-path-normalization§SSRF§Chain - text: OAuth DCR redirect + path normalization to read internal files

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `SSRF` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Advanced classic SSRF chain in modern OAuth/SaaS auth flows
- 适用场景：SaaS auth/API
...
- 本条技巧的价值点：Advanced classic SSRF chain in modern OAuth/SaaS auth flows
