...
# Derived technique from case: RCE via Group Access Validation Flaw in ssh-key-authority Open Source Tool

## 核心思路

Edit group rules with CSRF token to provision SSH keys to arbitrary servers

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Authorization (RCE)` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Classic access control bypass leads to full server takeover in SaaS infra provisioning tools
- 适用场景：SaaS infra tool
...
- 本条技巧的价值点：Classic access control bypass leads to full server takeover in SaaS infra provisioning tools
