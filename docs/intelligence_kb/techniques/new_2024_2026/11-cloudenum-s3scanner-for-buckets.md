...
vuln_class: misconfigs
source_url: https://github.com/amrelsagaei/Bug-Bounty-Hunting-Methodology-2025
source_author: Amr Elsagaei
...
# CloudEnum + S3Scanner for buckets

## 核心思路

cloud_enum -k target.com + s3scanner --access-key

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `misconfigs` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Finds exposed storage and misconfigs not tied to subdomains in BB programs
- 适用场景：cloud assets
...
- 本条技巧的价值点：Finds exposed storage and misconfigs not tied to subdomains in BB programs
