...
source_author: PortSwigger Research
...
risk_level: high
target_types:
  - web apps with H2 frontend downgrade
---

# HTTP/2 Request Tunnelling for Web Cache Poisoning (PortSwigger Lab Technique)

## 核心思路

Frontend HTTP/2 downgrade without header sanitization enables tunnelling to poison home page cache with alert

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `request smuggling` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Reliable cache poisoning via H2->H1 downgrade in labs and real stacks; BB high-signal
- 适用场景：web apps with H2 frontend downgrade
...
- 本条技巧的价值点：Reliable cache poisoning via H2->H1 downgrade in labs and real stacks; BB high-signal
