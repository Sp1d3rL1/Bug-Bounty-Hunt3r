...
source_url: https://www.bugcrowd.com/blog/a-guide-to-the-hidden-threat-of-prompt-injection/
source_author: Bugcrowd guest researchers
...
risk_level: high
target_types:
  - LLM SaaS chat-with-doc features
---

# Indirect Prompt Injection via PDF Upload in Chat-with-Doc Features

## 核心思路

Malicious": - /url: https://www.bugcrowd.com/blog/a-guide-to-the-hidden-threat-of-prompt-injection/§prompt_injection_data_leakage§Malicious - text: PDF with hidden PI ingested by RAG causes LLM to leak cloud creds (AWS metadata)

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `prompt_injection_data_leakage` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Real pen-test case in authorized BB program; shows untrusted doc ingestion risk in SaaS
- 适用场景：LLM SaaS chat-with-doc features
...
- 本条技巧的价值点：Real pen-test case in authorized BB program; shows untrusted doc ingestion risk in SaaS
...
6. 识别模型可访问的数据源、工具、插件、检索索引、用户/租户边界和输出通道。
7. 在自有数据/测试空间中验证 prompt injection、tool-call 越权、跨租户检索泄露。
8. 报告时强调信任边界、数据隔离和工具权限，而非“越狱”本身。
