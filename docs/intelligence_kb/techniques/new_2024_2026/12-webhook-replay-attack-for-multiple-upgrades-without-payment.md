...
vuln_class: replay without idempotency
...
risk_level: high
target_types:
  - SaaS webhook billing
---

# Webhook Replay Attack for Multiple Upgrades Without Payment

## 核心思路

re-send same payment.success webhook (no signature/event ID check) to trigger duplicate upgrades

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `replay without idempotency` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：allows multiple paid feature grants from single transaction in authorized lab/BB
- 适用场景：SaaS webhook billing
...
- 本条技巧的价值点：allows multiple paid feature grants from single transaction in authorized lab/BB
