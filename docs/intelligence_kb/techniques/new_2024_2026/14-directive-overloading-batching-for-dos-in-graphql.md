...
vuln_class: Overloading + Batching
...
source_author: MPGODMATCH (niche trick)
...
# Directive Overloading + Batching for DoS in GraphQL

## 核心思路

Repeat @aa directives 1000x combined with array batching to cause parser/resource exhaustion

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Overloading + Batching` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Tests server resilience to complex queries in lab/owned BB environments
- 适用场景：GraphQL APIs without query limits
...
- 本条技巧的价值点：Tests server resilience to complex queries in lab/owned BB environments
