...
# Environment Var Exfil via ps auxeww in Agent Tool Calls (Comment and Control variant)

## 核心思路

Agent": - /url: https://oddguan.com/blog/comment-and-control-prompt-injection-credential-theft-claude-code-gemini-cli-github-copilot/§prompt_injection_data_leakage_tool_calling§Agent - text: inherits full env; PI triggers process inspection to leak unfiltered secrets

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `prompt_injection_data_leakage_tool_calling` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Base64 evasion of secret scanners; high-signal for CI/CD agent BB programs
- 适用场景：GitHub Actions AI agents
...
- 本条技巧的价值点：Base64 evasion of secret scanners; high-signal for CI/CD agent BB programs
