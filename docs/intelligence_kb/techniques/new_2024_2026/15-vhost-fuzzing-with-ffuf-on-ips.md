...
vuln_class: host exposure
source_url: https://www.yeswehack.com/learn-bug-bounty/recon-series-recap-reconnaissance-footprinting
source_author: YesWeHack recon series
...
# VHost fuzzing with Ffuf on IPs

## 核心思路

ffuf -u - link "http://IP": - /url: http://IP - text: "-H \"Host: FUZZ.target.com\" -w vhosts.txt

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `host exposure` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Finds hidden vhosts on resolved IPs missed by DNS in BB recon
- 适用场景：web apps
...
- 本条技巧的价值点：Finds hidden vhosts on resolved IPs missed by DNS in BB recon
