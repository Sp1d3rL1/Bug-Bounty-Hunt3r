...
vuln_class: Role Validation
source_url: https://x.com/NitinGavhane_/status/2049573141626937609
source_author: Nitin Gavhane
source_date: 2026-04
...
# Role Context Validation Failure in Multi-Step Workflows

## 核心思路

Change role mid-workflow and inject role=admin in params

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Role Validation` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Exploits incomplete server-side checks in complex org permission flows
- 适用场景：SaaS multi-tenant
...
- 本条技巧的价值点：Exploits incomplete server-side checks in complex org permission flows
...
- https://x.com/NitinGavhane_/status/2049573141626937609
