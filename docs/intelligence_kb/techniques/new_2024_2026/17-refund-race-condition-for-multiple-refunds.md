...
source_url: https://osintteam.blog/series-business-logic-flaws-uncovered-14b3bd5ee60c
source_author: OSINT Team / BB patterns
source_date: 2025-10
...
risk_level: high
target_types:
  - e-commerce refund logic
---

# Refund Race Condition for Multiple Refunds

## 核心思路

fire rapid duplicate refund requests before backend state update

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `race condition` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：double or triple refunds on single transaction in ecom BB
- 适用场景：e-commerce refund logic
...
- 本条技巧的价值点：double or triple refunds on single transaction in ecom BB
