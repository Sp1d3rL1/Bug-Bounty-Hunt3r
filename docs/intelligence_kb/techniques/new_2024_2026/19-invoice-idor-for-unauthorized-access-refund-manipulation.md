...
vuln_class: invoice IDOR business logic
source_url: https://hackerone.com
source_author: BB hunter patterns (H1 style)
source_date: 2025
...
# Invoice IDOR for Unauthorized Access/Refund Manipulation

## 核心思路

guess/manipulate invoice ID to access or trigger refund on other user invoices

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `invoice IDOR business logic` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：cross-account billing abuse in authorized programs
- 适用场景：invoice/billing system
...
- 本条技巧的价值点：cross-account billing abuse in authorized programs
