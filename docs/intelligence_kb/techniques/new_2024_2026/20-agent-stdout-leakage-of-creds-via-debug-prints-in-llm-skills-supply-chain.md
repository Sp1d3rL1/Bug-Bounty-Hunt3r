...
vuln_class: data_leakage_tool_calling
source_url: https://x.com/ihtesham2005/status/
source_author: Ihtesham Ali (X hunter analysis)
...
risk_level: low
target_types:
  - LLM agent skill SaaS
---

# Agent Stdout Leakage of Creds via Debug Prints in LLM Skills (Supply Chain)

## 核心思路

Console.log/print() in agent skills captured by framework into LLM context for query leakage

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `data_leakage_tool_calling` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：76%+ skills vulnerable; no jailbreak needed; practical audit technique for agent marketplaces
- 适用场景：LLM agent skill SaaS
...
- 本条技巧的价值点：76%+ skills vulnerable; no jailbreak needed; practical audit technique for agent marketplaces
