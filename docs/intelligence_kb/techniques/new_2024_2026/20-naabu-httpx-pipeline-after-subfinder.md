...
vuln_class: service discovery
source_url: https://hacklido.com/blog/1359-my-complete-recon-workflow-for-bug-bounty-hunting-2025-edition
source_author: Purushotham
...
# Naabu + Httpx pipeline after Subfinder

## 核心思路

echo target | subfinder -silent | naabu -silent | httpx -title -tech-detect

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `service discovery` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Direct subdomain-to-port-to-live-host fingerprinting for quick vuln triage in BB
- 适用场景：web apps
...
- 本条技巧的价值点：Direct subdomain-to-port-to-live-host fingerprinting for quick vuln triage in BB
