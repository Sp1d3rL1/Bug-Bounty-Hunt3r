---
type: technique
category: new_method
derived_from_case: false
vuln_class: Override + Data Exfil
source_url: https://medium.com/@mpjani294/graphql-pentesting-for-bug-bounty-hunters-from-endpoint-discovery-to-high-impact-exploits-821f64a953b5
source_author: MPGODMATCH
source_date: 2025-11-15
collected_at: 2026-05-05
freshness: 2025
confidence: high
risk_level: low
target_types:
  - GraphQL with pagination
---

# Object Limit Overriding with Large Pagination in Queries

## 核心思路

Set limit:100000 on paginated fields discovered via schema to exfiltrate massive datasets

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Override + Data Exfil` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Bypasses default limits for bulk data theft in authorized BB programs
- 适用场景：GraphQL with pagination
- 技法拆解：先找到可控入口，再确认服务端信任边界，最后用最小化证据证明影响。
- 把该技巧拆成：入口发现、可控输入、信任边界、影响证明四段。
- 先找 lab/本地靶场复现，再映射到授权目标。

## 适用目标画像

- GraphQL with pagination

## 为什么有效

- 现代 Web/API 产品通常由多个前端、移动端、后台任务和第三方集成共同访问同一资源，容易出现某一路径缺少服务端校验。
- Bug Bounty 中高价值点通常不是单一 payload，而是“业务前提 + 可控对象 + 服务端信任边界 + 可证明影响”的组合。
- 本条技巧的价值点：Bypasses default limits for bulk data theft in authorized BB programs

## 手工验证流程（授权 / Lab only）

1. 确认 scope、禁止项、速率限制与是否允许该功能测试。
2. 准备至少两个自有测试账号；多租户场景准备两个 organization/workspace。
3. 先完整走通正常业务流，记录对象 ID、角色、状态、token、请求路径。
4. 只替换一个变量或一步状态，观察服务端响应和后续副作用。
5. 用最小必要数据证明影响；不读取、不导出第三方真实数据。
6. 把该技巧拆成：入口发现、可控输入、信任边界、影响证明四段。
7. 先找 lab/本地靶场复现，再映射到授权目标。

## 可自动化部分

- 可自动化收集 endpoint、参数、对象 ID 形态、历史 URL、JS 中的隐藏 API。
- 可自动化做“候选点标记”和“差异对比”，但越权、支付、账号状态影响必须手工确认。

## 误报/失败条件

- 只有客户端表现异常，没有服务端影响。
- 只能影响当前自有账号，无法证明跨权限、跨租户、财务、数据或流程影响。
- 目标业务前提不存在，或服务端已做完整对象归属/状态校验。
- 来源帖子/案例缺少可验证链接时，需降级为 review_queue 并二次确认。

## 授权边界

仅用于授权项目、靶场或自有环境。禁止无授权扫描、凭证滥用、爆破、DoS、真实支付损害、读取第三方真实隐私数据或绕过平台规则。

## 报告 impact 角度

- 说明攻击者前提、受影响对象、服务端缺失的校验，以及可造成的数据访问、权限提升、财务损失、业务流程绕过或租户隔离破坏。
- 证据只保留最小必要截图/请求响应，并打码 token、cookie、PII、支付信息。

## 相关案例链接

- https://medium.com/@mpjani294/graphql-pentesting-for-bug-bounty-hunters-from-endpoint-discovery-to-high-impact-exploits-821f64a953b5

<!-- GROK_EXPANSION_START -->

## Grok Expert 扩展补充

- [https://medium.com/@mpjani294/graphql-pentesting-for-bug-bounty-hunters-from-endpoint-discovery-to-high-impact-exploits-821f64a953b5](https://medium.com/@mpjani294/graphql-pentesting-for-bug-bounty-hunters-from-endpoint-discovery-to-high-impact-exploits-821f64a953b5)
vuln_class: Override + Data 验证暴露风险（不导出真实敏感数据） target_type: GraphQL with pagination confidence: high type: technique tags: [graphql, pagination, limit-bypass, data-exposure]

# Object Limit Overriding with Large Pagination in Queries
- **核心思路**
通过向 GraphQL 查询的分页参数（如 limit、first、last 等）传入异常大的整数值，验证服务器端是否真正强制执行对象数量上限。如果限制缺失，可揭示大规模数据暴露风险。
- **前置条件**
- 已发现 GraphQL endpoint 并通过 introspection/schema 暴露支持分页的参数字段。
- 仅限授权 Bug Bounty 程序或自有 Lab 环境。
- 目标字段为 list 类型（如 users、pastes、posts）且接受 Int 类型分页参数。
- **完整技法细节**
- 使用 introspection 查询确认支持 limit/first 等参数的字段。
- 构造查询并设置超大分页值（例如 limit: 100000），仅选择必要字段减少负载。
- 发送查询并观察响应：返回数据量、响应时间及是否触发服务器 cap。 示例（Lab only）：
```text
query {
pastes(limit: 100000, public: true) {
id
content
}
}
```
- **仅用于验证限制是否存在，绝不在生产环境执行真实大规模数据拉取**
。
- **适用目标画像**
采用 GraphQL 且依赖客户端分页参数的 API，常见于内容平台、用户生成内容系统或未严格实现 query cost 分析的后端服务。
- **为什么有效**
多数 GraphQL resolver 仅依赖框架默认限制（如 100 条），未对用户输入的 Int 值进行严格上界校验或 cost 分析，导致大值被直接接受并返回过多对象。
- **手工验证流程（授权 / Lab only）**
- 在授权 Lab 或 scope 内使用 GraphiQL/Altair 等客户端。
- 从小值（如 limit: 1000）开始逐步递增测试，监控响应大小、HTTP 状态及服务器行为。
- 记录实际返回记录数与预期默认限制的差异。
- 立即停止测试，避免任何生产影响。
- **可自动化部分**
Burp Suite 或自定义 Python 脚本批量对 schema 中所有 list 字段测试多种大 limit 值；结合 InQL / GraphQL Voyager 自动化字段发现。
- **误报/失败条件**
- 服务器启用 query complexity analyzer 或硬编码上限。
- Rate limiting / depth 限制拒绝大查询。
- 查询需认证且当前权限不足。
- **授权边界**
严格限定在 Bug Bounty 程序明确授权的 GraphQL endpoint，或自建 Lab 环境。禁止任何可能影响生产服务可用性的操作。
- **报告 impact 角度**
- 大规模敏感数据泄露（Mass Data Exposure）
- 潜在资源消耗风险
- 违反数据最小化原则（GDPR / 隐私合规）
- **相关案例链接**
- [https://medium.com/@mpjani294/graphql-pentesting-for-bug-bounty-hunters-from-endpoint-discovery-to-high-impact-exploits-821f64a953b5](https://medium.com/@mpjani294/graphql-pentesting-for-bug-bounty-hunters-from-endpoint-discovery-to-high-impact-exploits-821f64a953b5)

<!-- GROK_EXPANSION_END -->
