...
vuln_class: Secrets Exposure
source_url: https://medium.com/@thenewdate24/github-dorking-the-complete-2026-hunters-guide-to-finding-exposed-secrets-9a72331ed5bb
source_author: Deepanshu Deep
source_date: 2026-04
...
# GitHub Dorking for CI/CD Secrets in Workflows

## 核心思路

Dork .github/workflows for hardcoded keys/tokens in CI/CD YAML

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Secrets Exposure` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Quick wins on public repos in authorized bug bounty programs
- 适用场景：GitHub CI/CD
...
- 本条技巧的价值点：Quick wins on public repos in authorized bug bounty programs
