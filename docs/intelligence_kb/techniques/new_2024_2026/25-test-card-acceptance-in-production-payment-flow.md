...
# Test Card Acceptance in Production Payment Flow

## 核心思路

submit known test card numbers in live gateway checkout due to missing env checks

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `data leakage in prod` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：free paid features/subscriptions exposing integration misconfig in BB
- 适用场景：SaaS payment gateway
...
- 本条技巧的价值点：free paid features/subscriptions exposing integration misconfig in BB
