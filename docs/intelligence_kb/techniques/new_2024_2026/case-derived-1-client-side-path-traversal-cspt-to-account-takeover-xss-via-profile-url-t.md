...
# Derived technique from case: Client-Side Path Traversal (CSPT) to Account Takeover + XSS via profile URL traversal
...
- 来源要点：Chains file upload + unsanitized externalUrl to DOM XSS on Ctrl+Click; real BB $2k payout on authorized program
- 适用场景：web app (REST API frontend)
...
- 本条技巧的价值点：Chains file upload + unsanitized externalUrl to DOM XSS on Ctrl+Click; real BB $2k payout on authorized program
