...
# Derived technique from case: Akamai HTTP Request Smuggling via OPTIONS and Obsolete Line Folding (CVE-2025-32094)

## 核心思路

OPTIONS + obsolete line folding + Expect:100-continue desyncs parsing in body

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `HTTP request smuggling` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：High-impact smuggling on major CDN; led to large BB bounties and patches
- 适用场景：Akamai CDN
...
- 本条技巧的价值点：High-impact smuggling on major CDN; led to large BB bounties and patches
