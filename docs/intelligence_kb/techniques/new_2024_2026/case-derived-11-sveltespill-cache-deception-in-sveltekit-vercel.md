...
source_url: https://jorianwoltjer.com/blog/p/coding/cache-deception-on-my-new-site
source_author: Aikido / Jorian Woltjer
source_date: 2026-02
...
# Derived technique from case: SvelteSpill: Cache Deception in SvelteKit + Vercel

## 核心思路

__pathname= query param overrides path for immutable static cache key leaking session data

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `cache deception` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Leads to cross-user data leak via cached hidden posts; practical on Vercel-hosted Svelte
- 适用场景：SvelteKit/Vercel static sites
...
- 本条技巧的价值点：Leads to cross-user data leak via cached hidden posts; practical on Vercel-hosted Svelte
