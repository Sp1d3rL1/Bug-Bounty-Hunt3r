...
vuln_class: Query Batching
source_url: https://hackerone.com/reports/2166697
source_author: H1 Researcher
...
risk_level: medium
target_types:
  - HackerOne Internal GraphQL
---

# Derived technique from case: Bulk Report Submission via Named Query Batching in H1 GraphQL

## 核心思路

Use query name-based batching to submit multiple reports in one GraphQL operation

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Query Batching` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Bypasses per-operation limits for bulk actions in HackerOne's own GraphQL
- 适用场景：HackerOne Internal GraphQL
...
- 本条技巧的价值点：Bypasses per-operation limits for bulk actions in HackerOne's own GraphQL
