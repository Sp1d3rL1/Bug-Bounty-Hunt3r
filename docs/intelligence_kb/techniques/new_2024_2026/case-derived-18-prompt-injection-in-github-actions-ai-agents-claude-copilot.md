...
vuln_class: Actions Prompt Injection
source_url: https://www.theregister.com/2026/04/15/claude_gemini_copilot_agents_hijacked/
source_author: Guan et al.
...
# Derived technique from case: Prompt Injection in GitHub Actions AI Agents (Claude/Copilot)

## 核心思路

Craft PR title/issue comment to hijack AI agent and leak API keys

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Actions Prompt Injection` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Bounty payouts from Anthropic/Google/MS in authorized programs
- 适用场景：GitHub Actions AI
...
- 本条技巧的价值点：Bounty payouts from Anthropic/Google/MS in authorized programs
