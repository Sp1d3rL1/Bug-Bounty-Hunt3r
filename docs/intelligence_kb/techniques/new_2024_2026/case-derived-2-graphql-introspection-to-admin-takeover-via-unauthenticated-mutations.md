...
source_url: https://medium.com/meetcyber/graphql-introspection-to-admin-takeover-exploiting-unauthenticated-apis-2f4c14c3170b
source_author: Source_To_Sink
...
risk_level: medium
target_types:
  - Management Dashboard GraphQL API
---

# Derived technique from case: GraphQL Introspection to Admin Takeover via Unauthenticated Mutations

## 核心思路

Introspect schema to discover searchOperatorUserTable and createOperatorUser mutation then create ADMIN accounts

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `+ Broken Object Level Authorization` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Full dashboard takeover by creating privileged users with magic links in unauth GraphQL
- 适用场景：Management Dashboard GraphQL API
...
- 本条技巧的价值点：Full dashboard takeover by creating privileged users with magic links in unauth GraphQL
