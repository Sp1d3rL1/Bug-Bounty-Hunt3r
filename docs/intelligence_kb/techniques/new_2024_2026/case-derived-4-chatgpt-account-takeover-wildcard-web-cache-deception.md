...
source_url: https://nokline.github.io/bugbounty/2024/02/04/ChatGPT-ATO.html
source_author: nokline (Harel Security Research)
...
target_types:
  - ChatGPT
  - OpenAI (authorized BB)
---

# Derived technique from case: ChatGPT Account Takeover - Wildcard Web Cache Deception

## 核心思路

Path traversal URL parser confusion (%2F..%2F) caches /api/auth/session under /share/ path via CDN/origin diff

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `web cache deception` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Steals auth tokens for ATO; builds on prior Nagli report with $6500+ bounty potential
- 适用场景：ChatGPT/OpenAI (authorized BB)
...
- 本条技巧的价值点：Steals auth tokens for ATO; builds on prior Nagli report with $6500+ bounty potential
