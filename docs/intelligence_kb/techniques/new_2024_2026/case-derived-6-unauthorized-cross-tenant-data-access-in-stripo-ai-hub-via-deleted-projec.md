...
vuln_class: Improper Access Control
source_url: https://hackerone.com/reports/3459285
source_author: srcode
source_date: 2026-02
...
risk_level: high
target_types:
  - SaaS Email Design Workspace
---

# Derived technique from case: Unauthorized Cross-Tenant Data Access in Stripo AI Hub via Deleted Project

## 核心思路

Access deleted project data belonging to other tenant's AI Hub workspace

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Improper Access Control` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Critical tenant boundary breach in email design SaaS allowing data exfil
- 适用场景：SaaS Email Design Workspace
...
- 本条技巧的价值点：Critical tenant boundary breach in email design SaaS allowing data exfil
