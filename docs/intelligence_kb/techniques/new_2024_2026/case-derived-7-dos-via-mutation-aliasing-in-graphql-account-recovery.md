...
vuln_class: Aliasing + DoS
source_url: https://hackerone.com/reports/3287208
...
risk_level: medium
target_types:
  - Account Recovery GraphQL API
---

# Derived technique from case: DoS via Mutation Aliasing in GraphQL Account Recovery

## 核心思路

Alias verifyAccountRecoveryPhoneNumber mutation dozens of times in one request to exhaust resources

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Aliasing + DoS` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Causes sequential processing overload bypassing complexity checks in authorized H1 report
- 适用场景：Account Recovery GraphQL API
...
- 本条技巧的价值点：Causes sequential processing overload bypassing complexity checks in authorized H1 report
