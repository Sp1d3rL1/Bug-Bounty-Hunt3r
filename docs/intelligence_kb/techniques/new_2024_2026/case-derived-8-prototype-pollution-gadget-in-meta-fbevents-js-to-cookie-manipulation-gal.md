...
source_url: https://yinzhicao.org/ProbetheProto/FollowMyFlow.pdf
source_author: Z Kang et al. (GALA paper)
source_date: 2025
...
risk_level: low
target_types:
  - web tracking libs (Meta)
---

# Derived technique from case: Prototype Pollution Gadget in Meta fbevents.js to Cookie Manipulation (GALA research)

## 核心思路

Pollute undefined b[0] property flowing to document.cookie assignment

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `prototype pollution` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Meta paid BB bounty after responsible disclosure; turns PP into practical impact
- 适用场景：web tracking libs (Meta)
...
- 本条技巧的价值点：Meta paid BB bounty after responsible disclosure; turns PP into practical impact
