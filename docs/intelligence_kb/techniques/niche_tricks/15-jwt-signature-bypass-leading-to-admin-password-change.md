...
source_url: https://medium.com/@uzayransari14/how-i-accidentally-changed-the-admins-password-jwt-signature-bypass-leading-to-account-takeover-fe7657532974
source_author: Uzayr Ansari
source_date: 2026
...
risk_level: high
target_types:
  - JWT-protected admin panels
---

# JWT Signature Bypass Leading to Admin Password Change

## 核心思路

Modify admin claims in JWT + bypass sig verification via kid header or alg confusion

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Signature Bypass` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Accidental admin ATO via JWT tampering in real targets
- 适用场景：JWT-protected admin panels
...
- 本条技巧的价值点：Accidental admin ATO via JWT tampering in real targets
