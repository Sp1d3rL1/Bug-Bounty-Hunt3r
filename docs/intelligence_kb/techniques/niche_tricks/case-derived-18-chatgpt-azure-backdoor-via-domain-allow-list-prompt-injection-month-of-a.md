...
# Derived technique from case: ChatGPT Azure Backdoor via Domain Allow-List Prompt Injection (Month of AI Bugs)

## 核心思路

PI": - /url: https://www.theregister.com/2025/08/28/microsoft_copilot_copirate/§prompt_injection_tool_calling§PI - text: "tricks allow-list to enable unauthorized Azure API calls

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `prompt_injection_tool_calling` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Authorized Month of AI Bugs disclosure; shows tool calling bypass in SaaS
- 适用场景：OpenAI ChatGPT SaaS
...
- 本条技巧的价值点：Authorized Month of AI Bugs disclosure; shows tool calling bypass in SaaS
