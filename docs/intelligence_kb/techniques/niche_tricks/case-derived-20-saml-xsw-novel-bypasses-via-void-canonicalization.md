...
vuln_class: XML Signature Wrapping (XSW)
source_url: https://portswigger.net/research/the-fragile-lock
source_author: Zakhar Fedotkin (PortSwigger)
source_date: 2025-12
...
# Derived technique from case: SAML XSW Novel Bypasses via Void Canonicalization

## 核心思路

Namespace confusion + void C14N in libxml2 to make signature validate empty hash

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `XML Signature Wrapping (XSW)` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Bypasses in patched Ruby/PHP SAML libs post-2024 vulns
- 适用场景：Enterprise SAML SPs
...
- 本条技巧的价值点：Bypasses in patched Ruby/PHP SAML libs post-2024 vulns
