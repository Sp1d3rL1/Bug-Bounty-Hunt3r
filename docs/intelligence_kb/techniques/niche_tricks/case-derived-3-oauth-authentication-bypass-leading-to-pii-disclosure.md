...
# Derived technique from case: OAuth Authentication Bypass leading to PII disclosure
...
- 存在与 `Client Credentials Misuse` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Extracts 12MB+ customer PII via search endpoints in commercial platforms
- 适用场景：Bugcrowd private programs
...
- 本条技巧的价值点：Extracts 12MB+ customer PII via search endpoints in commercial platforms
