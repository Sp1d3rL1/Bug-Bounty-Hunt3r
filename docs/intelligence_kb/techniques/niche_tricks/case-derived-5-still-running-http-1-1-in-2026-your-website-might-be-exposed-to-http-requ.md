...
source_url: https://medium.com/@candywong_coffsec/still-running-http-1-1-in-2026-your-website-might-be-exposed-to-http-request-smuggling-attack-335633ca9766
source_author: Candy Wong
source_date: 2026-03
...
target_types:
  - modern HTTP
  - 1.1 stacks with proxies
  - CDNs
---

# Derived technique from case: Still Running HTTP/1.1 in 2026? Your Website Might Be Exposed to HTTP Request Smuggling Attack

## 核心思路

Conflicting CL + TE (zero chunk + hidden second GET) desyncs FE/BE parsers for queue poisoning

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `request smuggling (CL.TE)` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Bypasses WAFs, steals sessions/tokens, poisons cache with XSS/redirects at scale
- 适用场景：modern HTTP/1.1 stacks with proxies/CDNs
...
- 本条技巧的价值点：Bypasses WAFs, steals sessions/tokens, poisons cache with XSS/redirects at scale
