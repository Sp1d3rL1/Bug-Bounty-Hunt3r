...
risk_level: medium
target_types:
  - Enterprise Collaboration GraphQL API
---

# Derived technique from case: $1500 PII Leak via GraphQL Field-Level Permission Bypass

## 核心思路

Query suggestedCollaborators under Organization (public) instead of direct Project field to leak PII

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `Authorization Bypass` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Exploits inconsistent field-level auth on nested objects for sensitive data exfil in BB
- 适用场景：Enterprise Collaboration GraphQL API
...
- 本条技巧的价值点：Exploits inconsistent field-level auth on nested objects for sensitive data exfil in BB
