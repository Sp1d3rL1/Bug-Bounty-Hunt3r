...
vuln_class: None Algorithm Bypass
...
risk_level: high
target_types:
  - GraphQL + JWT web apps in bug bounty
---

# Derived technique from case: Full Workspace Account Takeover via JWT alg=none

## 核心思路

Change alg to none in JWT header + modify sub/owner claims + empty signature

## 前置条件

- 目标明确在授权 Bug Bounty scope 内，且 rules 未禁止该测试类别。
- 存在与 `None Algorithm Bypass` 相关的业务对象、接口、前端入口或集成链路。
- 使用自有测试账号、测试租户、sandbox 或平台允许的测试数据。
- 如果涉及支付/账号/隐私/生产状态变化，必须先确认项目允许并采用最小影响验证。

## 完整技法细节

- 来源要点：Hijack workspace owner via GraphQL ID leak + password reset
- 适用场景：GraphQL + JWT web apps in bug bounty
...
- 本条技巧的价值点：Hijack workspace owner via GraphQL ID leak + password reset
