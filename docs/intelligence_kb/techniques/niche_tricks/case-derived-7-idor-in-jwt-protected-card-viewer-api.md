...
# Derived technique from case: IDOR in JWT-Protected Card Viewer API
...
- 来源要点：Edge case where JWT present but no server-side ownership check
- 适用场景：Card viewer API
...
- 本条技巧的价值点：Edge case where JWT present but no server-side ownership check
